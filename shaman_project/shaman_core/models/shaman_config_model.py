        ...
        if isinstance(v, int):
            ...
            try:
                ...
                if v == "default":
                    ...
                    raise ValueError("Unknown input for max_step_duration variable.")
...
class ParameterRange(BaseModel):
    ...
    def check_range(cls, values):
        ...
        if values["max"] < values["min"]:
            raise ValueError(
                "Specified min value in parameter range is superior to max value."
            )
        return values
    ...
    def bbo_parameters(self) -> Dict:
        ...
        for param, value in bbo_parameters.items():
            if isinstance(value, str) and (
                ("bbo." in value) or ("sklearn." in value) or ("numpy." in value)
            ):
