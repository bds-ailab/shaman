...
class SHAManExperiment:
    ...
    def update_history(self, history: Dict) -> None:
        ...
        """
        logger.debug(f"Writing update dictionary {self._updated_dict(history)}")
    ...
    def fail(self):
        ...
        if self.experiment_id:
            request = self.api_client.put(f"experiments/{self.experiment_id}/fail")
            if not 200 <= request.status_code < 400:
                raise Exception(
    ...
    def summarize(self):
        ...
        print(f"Improvement compared to default:" f"{self.improvement_default}%")
        print(f"Optimal parametrization: {optimal_param}")
        print(f"Number of early stops: {sum(self.bb_optimizer.history['truncated'])}")
        print(
