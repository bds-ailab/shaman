...
class SurrogateModel(Heuristic):
    ...
    def __init__(self, regression_model, next_parameter_strategy, *args, **kwargs):
        ...
        """
        super(SurrogateModel, self).__init__(regression_model, next_parameter_strategy)
        ...
        if not hasattr(self.regression_model, "fit"):
            raise AttributeError(
                "Regression model argument must have a method named `fit` " "defined."
            )
    ...
    def regression_function(self, history):
        ...
        scaled_parameters = self.parameter_scaler.fit_transform(history["parameters"])
