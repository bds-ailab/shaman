...
def launch_experiment(experiment_form: ExperimentForm):
    ...
    config_file = str((CONFIG.running_directory / "config_shaman.cfg").absolute())
    shaman_config = ShamanConfig(SHAMAN_CONFIG_TEMPLATE, config_file)
    # Build the configuration file
    configuration_file = shaman_config.build_configuration(experiment_form.dict())
    # Write the sbatch from the request data
    sbatch_file = str((CONFIG.running_directory / "ui_sbatch.sbatch").absolute())
...
def get_experiments(
    ...
    """
    all_experiments = shaman_db["experiments"].find(projection=Experiment._project())
    if offset:
...
def get_experiment(experiment_id: str) -> Experiment:
    ...
    """
    experiment = shaman_db["experiments"].find_one({"_id": ObjectId(experiment_id)})
    return experiment
