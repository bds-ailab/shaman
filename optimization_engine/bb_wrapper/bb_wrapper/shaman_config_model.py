...
class BaseConfiguration:
    ...
    def from_yaml(cls, path, component_name):
        ...
        """
        return cls(**yaml.load(Path(path).read_text(), Loader=yaml.SafeLoader), component_name=component_name)
        ...
        if isinstance(v, int):
            ...
            try:
                ...
                if v == "default":
                    ...
                    raise ValueError(
                        "Unknown input for max_step_duration variable.")
...
class ParameterRange(BaseModel):
    ...
    def check_range(cls, values):
        ...
        if values["max"] < values["min"]:
            raise ValueError("Specified min value in parameter range is superior to max value."
                             )
        return values
...
class SHAManConfig(BaseConfiguration):
    ...
    def bbo_parameters(self) -> Dict:
        """
        Parses the bbo parameters to make them suitable to pass as argument of the BBOptimizer.
        """
        bbo_kwargs = dict()
        ...
        for param, value in bbo_parameters.items():
            if isinstance(value, str) and (('bbo.' in value) or ('sklearn.' in value) or ('numpy.' in value)):
