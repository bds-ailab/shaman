...
from bbo.initial_parametrizations import uniform_random_draw, latin_hypercube_sampling, \
    hybrid_lhs_uniform_sampling
from bbo.noise_reduction.resampling_policies import SimpleResampling, DynamicResamplingParametric, \
    DynamicResamplingParametric2, DynamicResamplingNonParametric, DynamicResamplingNonParametric2
from bbo.noise_reduction.fitness_transformation import FitnessTransformation, \
...
class BBOptimizer:
    ...
    def __init__(self,
        ...
        if not hasattr(self.black_box, "compute"):
            raise AttributeError(f"Object {black_box} does not have a method named `compute` "
                                 f"defined.")
        ...
        self.history = {"fitness": None, "parameters": None,
                        "truncated": None, "resampled": None, "initialization": None}
        ...
        if resampling_policy:
            try:
                self.resampling_policy = self.__resampling_policies__[
                    resampling_policy](**self.options)
            except KeyError:
                raise ValueError(
        ...
        if fitness_aggregation:
            try:
                self.fitness_aggregation = self.__fitness_aggregation__[
                    fitness_aggregation](**self.options)
            except KeyError:
                raise ValueError(
                    f"{resampling_policy} is not among possible choices."
    ...
    def _initialize(self, callbacks=[lambda x: x]):
        ...
        while step < self.initial_sample_size:
            ...
            self._optimization_step(
                parameter=initial_parameters[step])
    ...
    def _choose_next_parameters(self, current_parameters=None):
        ...
        parameter_returned_by_heuristic = self.heuristic.choose_next_parameter(
            aggregated_history, self.parameter_space, current_parameters=current_parameters)
        return self.closest_parameters(parameter_returned_by_heuristic, self.parameter_space)
    ...
    def _select_next_parameters(self, current_parameters=None):
        ...
        if self.max_retry > 1:
            cntr = 0
            while np.equal(self.history["parameters"], parameters).all(axis=1).any()\
    ...
    def summarize(self):
        ...
        if not self.launched:
            raise Exception("Black box has not been optimized yet. Please run the .optimize "
        ...
        print(
            f"Average variation within each parametrization: {self.measured_noise}")
    ...
    def measured_noise(self):
        ...
        for parametrization in np.unique(parameters_array, axis=0):
            noise_measurement.append(float(np.std(
    ...
    def size_explored_space(self):
        ...
        percentage_static_states = (len(self.history["parameters"]) - len(
            unique_parameters)) / len(self.history["parameters"]) * 100
