...
def exponential_schedule(initial_temperature, current_iteration, cooling_factor=0.1, **kwargs):
    """
    ...
    """
    assert cooling_factor < 1, f"Cooling factor must be < 1, you're warming up the system >.<"
    return cooling_factor ** current_iteration * initial_temperature


def logarithmic_schedule(initial_temperature, current_iteration, cooling_factor=10, **kwargs):
    """
    ...
    Returns:
        float: The new value for the temperature at round k.
    """
    assert cooling_factor > 1, f"Cooling factor must be bigger than 1, you're warming up the " \
                               f"system >.<"
    return initial_temperature/(1 + cooling_factor * np.log(current_iteration + 1))


def multiplicative_schedule(initial_temperature, current_iteration, cooling_factor=10, **kwargs):
    """
