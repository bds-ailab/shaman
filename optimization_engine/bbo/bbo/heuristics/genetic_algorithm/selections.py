...
def probabilistic_pick(history, *args, matingpool_size=100, elitism=False, **kwargs):
    ...
    mating_pool_parameters, mating_pool_fitness = _select_by_fitness(history, matingpool_size)
    # draw parent 1
    ...
    # else, draw according to the weighted probability rule
        ...
        parent_1 = mating_pool_parameters[np.random.choice(mating_pool_parameters.shape[0],
                                                           p=weighted_probability)]
    # draw parent 2
    # make sure that it is not possible to draw parent 1 again by computing the mating pool again
    mating_pool_parameters, mating_pool_fitness = _remove_from_history(mating_pool_parameters,
                                                                       mating_pool_fitness,
                                                                       parent_1)
    # compute the new weighted probability vector
    weighted_probability = _compute_weighted_probability(mating_pool_fitness)
    parent_2 = mating_pool_parameters[np.random.choice(mating_pool_parameters.shape[0],
                                                       p=weighted_probability)]
...
def _draw_fitness_parameters(parameters, fitness, number_of_individuals):
    ...
    """
    try:
        choices = np.random.choice(parameters.shape[0], number_of_individuals, replace=False)
...
def tournament_pick(history, *args, pool_size=10, matingpool_size=100, elitism=False, **kwargs):
    """
    ...
    # else, draw pool_size individuals and select the fittest
        ...
        parameters, _ = _sort_by_fitness(tournament_pool_parameters, tournament_pool_fitness)
