...
class SimpleResampling(ResamplingPolicy):
    ...
    def resample(self, history):
        ...
        return np.sum(np.all(parameters_array == last_elem, axis=1)) < self.nbr_resamples
...
class DynamicResampling(ResamplingPolicy):
    ...
    def __init__(self, percentage, resampling_schedule=None, allow_resampling_schedule=None, allow_resampling_start=1, *args, **kwargs):
        """
    ...
    def resample(self, history):
        ...
        Resample or not.
        """
        self.last_elem_fitness, self.last_elem_nbr, self.total_nbr = self.process_last_elem(
            history)
...
class DynamicResamplingParametric2(DynamicResamplingParametric):
    ...
    def allow_resampling(self, history):
        ...
        return np.median(self.last_elem_fitness) <= self.allow_resampling_schedule(self.total_nbr) * current_median
...
class DynamicResamplingNonParametric(DynamicResampling):
    ...
    def __init__(self, percentage, threshold, *args, **kwargs):
        ...
        if not 0 <= threshold <= 1:
            raise ValueError(
                "Threshold should be positive and inferior to 1.")
        super().__init__(percentage, *args, **kwargs)
    ...
    def get_interval_rank(self):
        ...
        """
        probs = 1 - 2 * \
            binom.cdf(np.arange(self.last_elem_nbr / 2),
                      self.last_elem_nbr, 1/2)
