...
class SimpleFitnessTransformation(FitnessTransformation):
    ...
    def transform(self, history):
        ...
        unique_parameterization_indexes = np.unique(parameters_array, axis=0, return_index=True)[1]
        unique_unsorted_parameterization = [parameters_array[index, :] \
        ...
        for parametrization in unique_unsorted_parameterization:
            ...
            new_fitness.append(self.estimator(
                fitness_array[np.all(parameters_array == parametrization, axis=1)]))
        return {"parameters": np.array(new_parameters), "fitness": np.array(new_fitness), "truncated": history["truncated"]}
