...
def uniform_random_draw(number_of_parameters, parameter_space):
    ...
    random_draw = [np.random.choice(axis, number_of_parameters, replace=True).tolist()
                   for axis in space]
...
def latin_hypercube_sampling(number_of_parameters, parameter_space):
    ...
    assert number_of_parameters <= smallest_size, "The number of parameters to be drawn can't be " \
