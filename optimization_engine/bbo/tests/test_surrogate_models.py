...
from bbo.heuristics.surrogate_models.regression_models import DecisionTreeSTDRegressor, CensoredGaussianProcesses

# Example of minimization function
from bbo.heuristics.surrogate_models.surrogate_models import SurrogateModel
from bbo.heuristics.surrogate_models.next_parameter_strategies import l_bfgs_b_minimizer, \
    cma_optimizer, compute_maximum_probability_improvement, maximum_probability_improvement, \
...
class TestAcquisitionFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gp_fun = SurrogateModel._build_prediction_function(gaussian_process, scaler)
    ...
    def test_compute_mpi(self):
        ...
        current_optimum = 5
        max_prob_imp = compute_maximum_probability_improvement(current_optimum, means, stds)
    ...
    def test_mpi(self):
        ...
        real_min = maximum_probability_improvement(self.gp_fun, ranges, fake_history['fitness'])
    ...
    def test_mpi_no_sd_option(self):
        ...
        with self.assertRaises(TypeError):
            maximum_probability_improvement(self.knn_fun, ranges, fake_history['fitness'])
...
class TestSurrogateModels(unittest.TestCase):
    ...
    def test_regression_model_no_fit(self):
        ...
        with self.assertRaises(AttributeError):
            SurrogateModel(regression_model=AintGotNoFit,
    ...
    def test_regression_model_no_predict(self):
        ...
        with self.assertRaises(AttributeError):
            SurrogateModel(regression_model=AintGotNoPredict,
                           next_parameter_strategy=expected_improvement)
    ...
    def test_choose_next_parameter_lbfgsb(self):
        ...
        real_new_parameter = surrogate_model.choose_next_parameter(fake_history, ranges)
        np.testing.assert_array_almost_equal(real_new_parameter, expected_new_parameter)
...
class TestCensoredBayesian(unittest.TestCase):
    ...
    def test_censored_bayesian(self):
        ...
        """
        expected_new_parameter = [4, 3]
        surrogate_model = SurrogateModel(regression_model=CensoredGaussianProcesses,
                                         next_parameter_strategy=expected_improvement)
        real_new_parameter = surrogate_model.choose_next_parameter(fake_history, ranges)
        np.testing.assert_array_equal(real_new_parameter, expected_new_parameter)
...
class TestDecisionTreeSTD(unittest.TestCase):
    ...
    def test_tree_ei(self):
        ...
        predictions_means, predictions_std = dtstdr.predict(combination_ranges, return_std=True)
        # Compute the EI and save the max
        current_optimum = np.min(fake_history["fitness"])
        computed_ei = compute_expected_improvement(current_optimum, predictions_means, predictions_std)
        ...
        surrogate_model = SurrogateModel(regression_model=DecisionTreeSTDRegressor,
                                         next_parameter_strategy=expected_improvement)
        next_parameter = surrogate_model.choose_next_parameter(fake_history, ranges)
