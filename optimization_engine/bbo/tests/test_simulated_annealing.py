...
from bbo.heuristics.simulated_annealing.cooldown_functions import exponential_schedule, \
    logarithmic_schedule, multiplicative_schedule
from bbo.heuristics.simulated_annealing.neighbor_functions import hop_to_next_value, random_draw
from bbo.heuristics.simulated_annealing.restart_functions import random_restart, threshold_restart
...
class TestCoolDownSchedules(unittest.TestCase):
    ...
    def test_exponential_schedule(self):
        ...
        actual_temperature = exponential_schedule(initial_temperature, current_iteration,
                                                  cooling_factor)
        self.assertEqual(expected_temperature, actual_temperature, "Exponential schedule did not "
    ...
    def test_logarithmic_schedule(self):
        ...
        actual_temperature = logarithmic_schedule(initial_temperature, current_iteration,
                                                  cooling_factor)
        self.assertEqual(expected_temperature, actual_temperature, "Logarithmic schedule did not "
    ...
    def test_multiplicative_schedule_wrong_factor(self):
        ...
        initial_temperature = 100
        with self.assertRaises(AssertionError):
            multiplicative_schedule(initial_temperature, current_iteration, cooling_factor)
    ...
    def test_multiplicative_schedule(self):
        ...
        actual_temperature = multiplicative_schedule(initial_temperature, current_iteration,
                                                     cooling_factor)
        self.assertEqual(expected_temperature, actual_temperature, "Multiplicative schedule did "
                                                                   "not "
                                                                   "return expected value.")
...
class TestNeighboringFunction(unittest.TestCase):
    ...
    def test_hop_next_value_center_grid(self):
        ...
        """
        np.random.seed(10)
        fake_grid = np.array([np.arange(-10, 10, 1), np.arange(-10, 10, 1), np.arange(-10, 11, 1)])
        current_parameter = np.array([0, 0, 0])
        next_value = hop_to_next_value(current_parameter, fake_grid)
        expected_value = np.array([-1, -1, 1])
        np.testing.assert_array_equal(next_value, expected_value, "Neighboring parameter was not "
                                                                  "the expected one.")
    ...
    def test_hop_next_value_lower_border_grid(self):
        ...
        np.testing.assert_array_equal(expected_next_value, next_value, "Expected and real value "
...
class TestRestartMethods(unittest.TestCase):
    ...
    def test_random_restarts(self):
        ...
        t_max = 10
        real_restart, real_t_max = random_restart(bernouilli_parameter=bernouilli_parameter,
                                                  initial_temperature=t_max)
        self.assertTrue(real_restart)
        self.assertEqual(t_max, real_t_max, "Maximal temperature was not computed properly.")
    ...
    def test_threshold_restarts(self):
        ...
        t_max = 10
        real_restart, real_t_max = threshold_restart(probability_threshold=probability_threshold,
...
class TestSimulatedAnnealing(unittest.TestCase):
    ...
    def test_simulated_annealing_wrong_temp(self):
        ...
        with self.assertRaises(TypeError):
            SimulatedAnnealing(initial_temperature="tutu",
    ...
    def test_simulated_annealing_no_restart(self):
        ...
        sa = SimulatedAnnealing(initial_temperature=50,
                                neighbor_function=hop_to_next_value,
                                cooldown_function=multiplicative_schedule,
                                cooling_factor=2,
                                restart=False)
    ...
    def test_simulated_annealing_temp_stop(self):
        ...
        np.random.seed(10)
        ranges = np.array([np.arange(20), np.arange(20)])
        sa = SimulatedAnnealing(initial_temperature=0.1,
                                neighbor_function=hop_to_next_value,
                                cooldown_function=multiplicative_schedule,
                                cooling_factor=10,
                                restart=threshold_restart,
