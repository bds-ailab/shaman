...
from bbo.noise_reduction.resampling_policies import ResamplingPolicy, SimpleResampling, \
    DynamicResampling, DynamicResamplingParametric, DynamicResamplingNonParametric, DynamicResamplingNonParametric2


def test_resampling_schedule(nbr_it):
...
class TestDynamicResampling(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        """
        dynamic_resampling = DynamicResampling(percentage=.5)
    ...
    def test_resampling_schedule_None(self):
        ...
        dynamic_resampling = DynamicResampling(percentage=.5)
        self.assertEqual(.5,
                         dynamic_resampling.resampling_schedule(1))
    ...
    def test_resampling_schedule(self):
        ...
        dynamic_resampling = DynamicResampling(
            percentage=.5, resampling_schedule=test_resampling_schedule)
        self.assertEqual(.5/np.log(2),
                         dynamic_resampling.resampling_schedule(1))
    ...
    def test_allow_resampling_schedule(self):
        ...
        """
        dynamic_resampling = DynamicResampling(
            percentage=.5, allow_resampling_start=5, allow_resampling_schedule=test_resampling_schedule)
        self.assertEqual(
...
class TestDynamicResamplingParametric(unittest.TestCase):
    ...
    def test_dynamic_resampling_formula(self):
        ...
        history = {"fitness": np.array([10, 5, 4, 14, 15, 16]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1]])}
        fitness = np.array(history["fitness"])
        parameters = np.array(history["parameters"])
        last_elem_fitness = fitness[np.all(
            parameters == parameters[-1], axis=1)]
    ...
    def test_dynamic_resampling_resample(self):
        ...
        history = {"fitness": np.array([10, 5, 4, 12, 15, 20]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1]])}
    ...
    def test_dynamic_resampling_schedule(self):
        ...
        """
        history = {"fitness": np.array([10, 5, 4, 10, 11, 11.5]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1]])}
        ...
        test_dynamic_resampling = DynamicResamplingParametric(
            0.2, resampling_schedule=test_resampling_schedule)
        self.assertTrue(test_dynamic_resampling.resample(history))
...
class TestDynamicNonParametricResampling(unittest.TestCase):
    ...
    def test_dynamic_resampling_non_parametric_init(self):
        ...
        """
        with self.assertRaises(ValueError):
            DynamicResamplingNonParametric(percentage=-0.1, threshold=.9)
        with self.assertRaises(ValueError):
    ...
    def test_dynamic_resampling_non_parametric_one_resample(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 12]),
    ...
    def test_dynamic_resampling_non_parametric_no_resample(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 12, 12, 12, 12]),
    ...
    def test_dynamic_resampling_non_parametric_resample(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 8, 10, 12, 16]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1], [2, 1]])}
        self.assertTrue(test_dynamic_resampling.resample(history))
    ...
    def test_dynamic_resampling_effect_threshold(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.95)
        history = {"fitness": np.array([10, 5, 4, 8, 10, 12, 16, 15, 10, 12, 16, 14]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1]])}
        self.assertTrue(test_dynamic_resampling.resample(history))
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.5)
        self.assertFalse(test_dynamic_resampling.resample(history))
    ...
    def test_dynamc_resamplingschedule(self):
        ...
        """
        test_dynamic_resampling = DynamicResamplingNonParametric(
            0.2, threshold=.95, resampling_schedule="")
        history = {"fitness": np.array([10, 5, 4, 8, 10, 12, 16, 15, 10, 12, 16, 14]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1]])}
...
class TestDynamicNonParametricResampling2(unittest.TestCase):
    ...
    def test_dynamic_resampling_non_parametric_init(self):
        ...
        """
        with self.assertRaises(ValueError):
            DynamicResamplingNonParametric2(percentage=-0.1, threshold=.9)
        with self.assertRaises(ValueError):
    ...
    def test_dynamic_resampling_non_parametric_one_resample(self):
        ...
        is automatically repeated.
        """
        test_dynamic_resampling = DynamicResamplingNonParametric2(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 12]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1]])}
        self.assertTrue(test_dynamic_resampling.resample(history))
    ...
    def test_dynamic_resampling_non_parametric_no_resample(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric2(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 12, 12]),
    ...
    def test_dynamic_resampling_non_parametric_resample2(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric2(
            0.2, threshold=.9)
        history = {"fitness": np.array([10, 5, 4, 2, 5, 3, 7]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1], [2, 1]])}
        self.assertTrue(test_dynamic_resampling.resample(history))
    ...
    def test_dynamic_resampling_effect_threshold(self):
        ...
        test_dynamic_resampling = DynamicResamplingNonParametric2(
            0.2, threshold=.95)
        history = {"fitness": np.array([20, 5, 4, 8, 10, 12, 16, 15, 10, 12, 16, 13]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1], [2, 1]])}
        self.assertTrue(test_dynamic_resampling.resample(history))
        test_dynamic_resampling = DynamicResamplingNonParametric2(
            0.2, threshold=.5)
