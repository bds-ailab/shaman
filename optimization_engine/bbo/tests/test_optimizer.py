...
def mock_callback_1(history):
    ...
    parameters_transform = np.sum(history["parameters"])
...
def mock_callback_2(history):
    ...
    parameters_transform = np.sum(history["parameters"])**2
    print(f"Result: {fitness_transform} + {parameters_transform}")


# parameter space
parameter_space = np.array(
    [np.arange(-5, 5, 1), np.arange(-6, 6, 1), np.arange(-6, 6, 1)]).T
# maximum number of iterations
nbr_iteration = 5
# maximum elapsed time
time_out = 100
# create fake history
fake_history = {"fitness": np.array([10, 5, 4, 2, 15, 20]),
...
class TestOptimizer(unittest.TestCase):
    ...
    def test_no_compute_method(self):
        ...
        with self.assertRaises(AttributeError):
            BBOptimizer(black_box=self.no_compute,
    ...
    def test_transform_function(self):
        ...
        """
        def perf_function(x): return x + 2
        bb_obj = BBOptimizer(black_box=self.parabola,
                             heuristic="surrogate_model",
                             max_iteration=nbr_iteration,
                             parameter_space=parameter_space,
        ...
        self.assertEqual(bb_obj.compute_result(np.array([5, 0])), 27, "Use of a transformation "
    ...
    def test_incorrect_heuristic_name(self):
        ...
        with self.assertRaises(ValueError):
            BBOptimizer(black_box=self.parabola,
    ...
    def test_append_parameters(self):
        ...
        np.testing.assert_array_equal(bb_obj.history["parameters"], np.array([[1, 2], [2, 3],
                                                                              [1, 3], [
                                                                                  4, 3],
                                                                              [2, 1], [
                                                                                  1, 5],
    ...
    def test_append_performance(self):
        ...
        np.testing.assert_array_equal(bb_obj.history["fitness"], np.array(np.array([10, 5,
                                                                                    4, 2,
                                                                                    15, 20,
                                                                                    10])))
    ...
    def test_stop_rule_false(self):
        ...
        self.assertFalse(bb_obj.stop_rule, "Exceeded number of iteration did not stop the "
                                           "optimizing loop.")
        ...
        self.assertFalse(bb_obj.stop_rule,
                         "Exceeded elapsed time did not stop optimizing loop.")
        ...
        self.assertFalse(bb_obj.stop_rule, "Internal heuristic stop did not stop the optimizing "
                                           "loop.")
    ...
    def test_stop_rule_true(self):
        ...
        # If the number of iterations is exceeded
        bb_obj.nbr_iteration = nbr_iteration - 1
        self.assertTrue(bb_obj.stop_rule, "Exceeded number of iteration did not stop the "
                                          "optimizing loop.")
        ...
        # If the time elapsed is exceeded
        bb_obj.elapsed_time = time_out - 1
        self.assertTrue(bb_obj.stop_rule,
                        "Exceeded elapsed time did not stop optimizing loop.")
        ...
        self.assertTrue(bb_obj.stop_rule, "Internal heuristic stop did not stop the optimizing "
                                          "loop.")
    ...
    def test_evaluate_fitness_single_item(self):
        ...
        np.testing.assert_array_equal(expected_result, result, "Fitness evaluation on a parameter "
    ...
    def test_total_iteration(self):
        ...
        bb_obj = BBOptimizer(black_box=self.parabola,
                             ...
                             regression_model=GaussianProcessRegressor)
        bb_obj.optimize()
        self.assertEqual(nbr_iteration + 2, bb_obj.total_iteration, "Total number of iterations "
    ...
    def test_closest_parameter(self):
        ...
        np.testing.assert_array_equal(expected_parameter, actual_parameter,
    ...
    def test_size_explored_space(self):
        ...
        """
        history = {"fitness": np.array([10, 5, 4]),
        ...
        self.assertEqual(expected_static_moves, real_static_moves, "Percentage of static moves "
                                                                   "was not computed properly.")
        self.assertEqual(expected_explored_space, real_explored_space, "Percentage of explored "
                                                                       "space was not computed "
                                                                       "properly.")
    ...
    def test_local_exploration_cost(self):
        ...
        """
        history = {"fitness": np.array([10, 5, 6, 2, 15, 20]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [4, 3], [2, 1], [1, 5]]),
                   "truncated": np.array([True, True, True, True, True, True])}
        ...
        self.assertEqual(expected_number_states, real_number_states, "Number of local regressions "
                                                                     "was not correctly computed.")
        self.assertEqual(expected_performance_cost, real_performance_cost, "Loss due to "
                                                                           "regression was not "
                                                                           "correctly computed.")
    ...
    def test_global_exploration_cost(self):
        ...
        """
        history = {"fitness": np.array([10, 5, 6, 2, 15, 4]),
                   "parameters": np.array([[1, 2], [2, 3], [1, 3], [4, 3], [2, 1], [1, 5]]),
                   "truncated": np.array([True, True, True, True, True, True])}
        ...
        self.assertEqual(expected_number_states, real_number_states, "Percentage of static moves "
                                                                     "was not computed properly.")
        self.assertEqual(expected_performance_cost, real_performance_cost, "Percentage of explored "
                                                                           "space was not computed "
                                                                           "properly.")
    ...
    def test_fitness_gain_per_iteration(self):
        ...
        np.testing.assert_array_equal(expected_gain_per_iteration,
                                      bb_obj.fitness_gain_per_iteration,
                                      "Computation of fitness gain per iteration did not work as "
                                      "expected.")
    ...
    def test_reevaluation(self):
        ...
        bb_obj = BBOptimizer(black_box=self.parabola,
                             ...
                             max_retry=5)
        bb_obj.optimize()
        self.assertEqual(np.unique(bb_obj.history['parameters'], axis=0).shape[0], 7,
    ...
    def test_reset(self):
        ...
        self.assertDictEqual(
            bb_obj.history, {"fitness": None, "parameters": None})
    ...
    def test_select_parameters(self):
        ...
        np.testing.assert_array_equal(parameter, np.array([-5, -6, -6]))
    ...
    def test_optimization_step_callback(self):
        ...
        bb_obj = BBOptimizer(black_box=self.parabola,
                             ...
                             regression_model=GaussianProcessRegressor)
        test_parameter = np.array([10, 10, 10])
        bb_obj._optimization_step(test_parameter, callbacks=[
                                  lambda x: print(x["fitness"])])
        # Redirect the stdout to not mess with the system
        sys.stdout = sys.__stdout__
        expected = "[200]\n"
        self.assertEqual(expected, captured_output.getvalue())
    ...
    def test_optimizer_simple_resampling_resample(self):
        ...
        bb_obj = BBOptimizer(black_box=self.parabola,
                             ...
                             nbr_resamples=3)
        bb_obj.history = {"fitness": np.array([10, 5, 4, 2, 15, 20]),
                          "parameters": np.array([[1, 2, 3], [2, 3, 3], [1, 3, 3], [1, 5, 3], [1, 1, 3], [1, 5, 3]]),
                          "truncated": np.array([True, True, True, True, True, True])}
        parameter = bb_obj._select_next_parameters()
    ...
    def test_optimizer_simple_resampling_noresample(self):
        ...
        bb_obj.history = {"fitness": np.array([10, 5, 4, 2, 15, 20]),
                          ...
                          "truncated": np.array([True, True, True, True, True, True])}
        parameter = bb_obj._select_next_parameters()
        np.testing.assert_array_equal(parameter, [2, 3, 3])
    ...
    def test_optimizer_process_simple_resampling(self):
        ...
        bb_obj = BBOptimizer(black_box=self.parabola,
                             ...
                             nbr_resamples=2)
        bb_obj.optimize()
        np.testing.assert_array_equal(bb_obj.history["fitness"], np.array([
                                      16., 61., 61., 16.,  9.,  9.,  0.]))
    ...
    def test_fitness_aggregation_std(self):
        ...
        aggregated_history = bb_obj.fitness_aggregation.transform(
            bb_obj.history)
        np.testing.assert_array_equal(
            aggregated_history["fitness"], np.array([0., 0., 0., 9., 0.]))
        np.testing.assert_array_equal(aggregated_history["parameters"], np.array([[1, 2, 3],
                                                                                  [2, 3, 3],
                                                                                  [1, 3, 3],
                                                                                  [1, 5, 3],
                                                                                  [1, 1, 3]]))
