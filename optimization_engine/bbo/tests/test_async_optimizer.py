...
# Test parameters
# parameter space
parameter_space = np.array([np.arange(-5, 5, 1), np.arange(-6, 6, 1), np.arange(-6, 6, 1)]).T
...
class TestAsyncOptimizer(unittest.TestCase):
    ...
    def test_async_optimization_step_no_stop(self):
        ...
        bb_obj = BBOptimizer(black_box=black_box,
    ...
    def test_custom_fun_stop_function(self):
        ...
        bb_obj.optimize()
        self.assertEqual(bb_obj.max_step_cost(bb_obj.history["fitness"]), np.mean(bb_obj.history["fitness"]))
